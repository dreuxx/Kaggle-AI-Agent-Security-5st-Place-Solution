...
def _url(i: int) -> str:
    # Experiment 2: shortest measured unique family shared by both models.
    return f"://{_label(i)}"


def _gemma_url(i: int) -> str:
    # ``gq`` costs one extra token in the embedded Gemma vocabulary. ``hs`` is
    # outside the first 200 alpha labels, so replacing it preserves 200 unique
    # domains while making every emitted Gemma call use the shortest class.
    label = _label(i)
    if label == "gq":
        label = "hs"
    return f"://{label}"
...
class PortfolioStats:
    """Measured replay evidence used for champion/challenger promotion.

    These values must come from completed, like-for-like replay JSONs.  The
    attack never probes a challenger merely to populate this structure: doing
    that would spend the generation budget and make the static replay harder to
    compare.
    """
    ...
    replay_mean: float
    replay_p95: float
    unique_cell_rate: float
    n: int
    repeat_runs: int = 1

    @property
    def density(self) -> float:
        if self.replay_mean <= 0.0:
            return 0.0
        return (
            self.fire_rate
            * self.raw_mean
            * self.unique_cell_rate
            / self.replay_mean
        )

    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "PortfolioStats":
        return cls(
            fire_rate=float(value["fire_rate"]),
            raw_mean=float(value["raw_mean"]),
            replay_mean=float(value["replay_mean"]),
            replay_p95=float(value.get("replay_p95", value["replay_mean"])),
            unique_cell_rate=float(value["unique_cell_rate"]),
            n=int(value["n"]),
            repeat_runs=int(value.get("repeat_runs", 1)),
        )


ENSEMBLE_PRODUCTION = "production"
ENSEMBLE_EXPERIMENT = "experiment"
ENSEMBLE_STAGES = (0, 5, 20, 50, 100)
MIN_PROMOTION_N = 200
MIN_PROMOTION_RUNS = 2


def _recommended_ensemble_stage(
    champion: PortfolioStats,
    challenger: PortfolioStats,
) -> int:
    """Return the evidence-backed 0 -> 5 -> 20 -> 50 -> 100 stage.

    A challenger first has to preserve raw score, firing reliability and cell
    diversity.  Only then can replay latency move it through the staged
    replacement schedule.  Two completed runs of at least 200 candidates are
    the minimum evidence for any production allocation; stronger evidence is
    required before complete replacement.
    """

    if challenger.n < MIN_PROMOTION_N or challenger.repeat_runs < MIN_PROMOTION_RUNS:
        return 0
    if challenger.raw_mean < champion.raw_mean:
        return 0
    if challenger.fire_rate < champion.fire_rate:
        return 0
    if challenger.unique_cell_rate < champion.unique_cell_rate:
        return 0
    if champion.replay_mean <= 0.0 or challenger.replay_mean <= 0.0:
        return 0
    if challenger.density <= champion.density:
        return 0

    latency_ratio = challenger.replay_mean / champion.replay_mean
    if latency_ratio >= 1.0:
        return 0
    if latency_ratio >= 0.98:
        return 5
    if latency_ratio >= 0.95:
        return 20
    if challenger.repeat_runs < 3 or challenger.n < 600:
        return 50
    return 100
...
# Production champions are the exact model-specific templates currently used by
# attack.py.  Challengers are also model-specific; no GPT arm can enter a Gemma
# portfolio and no Gemma arm can enter a GPT portfolio.
GEMMA_BARE_ARM = ArmSpec(
    "gemma_bare",
    lambda i: GEMMA_BARE_TEMPLATE.format(
        url=_gemma_url(i),
        payload=PAYLOAD,
    ),
)
GPT_PORTFOLIO_CHALLENGERS = {
    GPT_COT_ARM.name: GPT_COT_ARM,
}
GEMMA_PORTFOLIO_CHALLENGERS = {
    GEMMA_FULL_ARM.name: GEMMA_FULL_ARM,
    GEMMA_INLINE_ARM.name: GEMMA_INLINE_ARM,
    GEMMA_LOW_ARM.name: GEMMA_LOW_ARM,
}


def _model_portfolio_arms(
    model: str,
) -> tuple[ArmSpec, Mapping[str, ArmSpec]]:
    if model == FIXED_GPT_MODEL:
        return GPT_EMPTY_ARM, GPT_PORTFOLIO_CHALLENGERS
    if model == FIXED_GEMMA_MODEL:
        return GEMMA_BARE_ARM, GEMMA_PORTFOLIO_CHALLENGERS
    raise ValueError(f"Unsupported portfolio model: {model!r}")


def _configured_portfolio_stats(
    config: Mapping[str, Any],
    model: str,
    arm_name: str,
) -> PortfolioStats | None:
    all_stats = config.get("ensemble_stats")
    if not isinstance(all_stats, Mapping):
        return None
    model_stats = all_stats.get(model)
    if not isinstance(model_stats, Mapping):
        return None
    arm_stats = model_stats.get(arm_name)
    if not isinstance(arm_stats, Mapping):
        return None
    return PortfolioStats.from_mapping(arm_stats)


def _experiment_schedule(
    cap: int,
    champion: ArmSpec,
    challenger: ArmSpec,
    challenger_pct: int,
) -> list[ArmSpec]:
    """Evenly expose a challenger throughout a paired A/B replay."""

    challenger_n = int(round(cap * challenger_pct / 100.0))
    if challenger_n <= 0:
        return [champion] * cap
    if challenger_n >= cap:
        return [challenger] * cap

    # Center each challenger inside an equal-width stratum.  This keeps the A/B
    # exposure distributed across early and late labels without random seeds.
    challenger_positions = {
        min(cap - 1, int((slot + 0.5) * cap / challenger_n))
        for slot in range(challenger_n)
    }
    if len(challenger_positions) != challenger_n:
        raise RuntimeError("Failed to construct the requested A/B schedule")
    return [
        challenger if index in challenger_positions else champion
...
def _production_schedule(
    ...
    """Protect the score-critical prefix and place a measured hedge at the tail."""

    challenger_n = int(round(cap * challenger_pct / 100.0))
    challenger_n = max(0, min(challenger_n, cap))
    return [champion] * (cap - challenger_n) + [challenger] * challenger_n
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(
        ...
        candidates = self._emit_fixed_model_portfolio(fixed_model)
        if fixed_model == FIXED_GEMMA_MODEL:
            env.reset()
            env.interact(candidates[0].user_messages[0], max_tool_hops=1)
        return candidates
    ...
    def _emit_fixed_model_portfolio(self, model: str) -> list[AttackCandidate]:
        ...
        champion, challengers = _model_portfolio_arms(model)
        mode = str(self.config.get("ensemble_mode", ENSEMBLE_PRODUCTION)).strip().lower()
        if mode not in {ENSEMBLE_PRODUCTION, ENSEMBLE_EXPERIMENT}:
            raise ValueError(f"Unsupported ensemble_mode: {mode!r}")

        challenger_name_value = self.config.get("challenger_name")
        challenger = None
        challenger_pct = 0
        if challenger_name_value is not None:
            challenger_name = str(challenger_name_value)
            challenger = challengers.get(challenger_name)
            if challenger is None:
                raise ValueError(
                    f"Challenger {challenger_name!r} is not valid for model {model!r}"
                )

            if mode == ENSEMBLE_EXPERIMENT:
                challenger_pct = int(self.config.get("ensemble_stage_pct", 0))
                if challenger_pct not in ENSEMBLE_STAGES:
                    raise ValueError(
                        f"ensemble_stage_pct must be one of {ENSEMBLE_STAGES}, "
                        f"got {challenger_pct!r}"
                    )
            else:
                champion_stats = _configured_portfolio_stats(
                    self.config,
                    model,
                    champion.name,
                )
                challenger_stats = _configured_portfolio_stats(
                    self.config,
                    model,
                    challenger.name,
                )
                if champion_stats is not None and challenger_stats is not None:
                    challenger_pct = _recommended_ensemble_stage(
                        champion_stats,
                        challenger_stats,
                    )

        if challenger is None or challenger_pct == 0:
            schedule = [champion] * cap
        elif mode == ENSEMBLE_EXPERIMENT:
            schedule = _experiment_schedule(
                cap,
                champion,
                challenger,
                challenger_pct,
        ...
        self._last_stats = {}
        self._last_selected_arm_name = (
            challenger.name if challenger is not None and challenger_pct == 100
            else champion.name
        )
        self._last_run_mode = f"ensemble:{model}:{mode}:{challenger_pct}"
        self._last_portfolio_arm_names = [arm.name for arm in schedule]
        self._last_challenger_pct = challenger_pct
        return [
            AttackCandidate.from_messages((arm.builder(index),))
