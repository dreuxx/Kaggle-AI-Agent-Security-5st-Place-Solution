...
OUTPUT = ROOT / "sandbox/output/gemma_preturn_reset_exact_20260828.json"
REPEATS = tuple(range(9))
...
def main() -> None:
    ...
    normal_id = llm.tokenize(
        NORMAL_POST.encode(), add_bos=False, special=True
    )[0]
    stop_ids = llm.tokenize(
        DIRECT_STOP.encode(), add_bos=False, special=True
    )
    if len(stop_ids) != 1:
        raise RuntimeError(f"Expected one direct-stop token, got {stop_ids}")
    ...
    for repeats in REPEATS:
        candidates = attack.AttackAlgorithm(
            config={
                "fixed_model": "gemma",
                "n_candidates": 200,
                "gemma_preturn_reset_repeats": repeats,
            }
        ).run(None, AttackRunConfig(time_budget_s=1.0, max_tool_hops=8))
        if len(candidates) != 200:
            raise RuntimeError(f"repeat={repeats}: expected 200 candidates")
        messages = [tuple(candidate.user_messages) for candidate in candidates]
        if len(set(messages)) != 200 or any(len(message) != 1 for message in messages):
            raise RuntimeError(f"repeat={repeats}: invalid candidate contract")
        ...
        initial_rendered = _render_hf_messages(history, profile=profile)
        initial_prompt = formatter(messages=initial_rendered, tools=tools).prompt
        ...
        post_rendered = _render_hf_messages(post_history, profile=profile)
        post_prompt = formatter(messages=post_rendered, tools=tools).prompt
        post_ids = llm.tokenize(
            post_prompt.encode(), add_bos=False, special=True
        )
        ...
        rows.append(
            {
                ...
                "user_tokens": len(
                    llm.tokenize(
                        user_message.encode(), add_bos=False, special=True
                    )
                ),
