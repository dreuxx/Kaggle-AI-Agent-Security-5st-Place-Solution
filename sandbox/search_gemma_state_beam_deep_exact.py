...
SOURCE = ROOT / "sandbox/output/gemma_state_beam_exact_20260828.json"
OUTPUT = ROOT / "sandbox/output/gemma_state_beam_deep_exact_20260828.json"
...
def main() -> None:
    ...
    if not SOURCE.is_file():
        raise SystemExit(f"Missing source beam: {SOURCE}")

    source = json.loads(SOURCE.read_text(encoding="utf-8"))
    atoms = tuple(str(atom) for atom in source["search"]["atoms"])
    source_rows = [
        row for row in source["rows"] if int(row["depth"]) == START_DEPTH
    ]
    source_control_tokens = next(
        int(row["user_tokens"]) for row in source["rows"] if int(row["depth"]) == 0
    )
    source_rows.sort(
        key=lambda row: (
            float(row["post_stop_vs_normal_gap"])
            - 0.01 * (int(row["user_tokens"]) - source_control_tokens),
    ...
    seed_paths = [
        tuple(int(index) for index in row["atom_path"])
        for row in source_rows[:BEAM_WIDTH]
    ]
    ...
    for path in seed_paths:
        prefix = "".join(atoms[index] for index in path)
        evaluate(prefix, START_DEPTH, path)
        beam.append((prefix, path))
