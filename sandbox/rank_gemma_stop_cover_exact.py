...
OUTPUT = ROOT / "sandbox/output/gemma_stop_cover_exact_20260828.json"
REPEATS = tuple(range(25))
...
def main() -> None:
    ...
    for repeats in REPEATS:
        candidates = attack.AttackAlgorithm(
            config={
                "fixed_model": "gemma",
                "n_candidates": 200,
                "gemma_stop_steer_repeats": repeats,
            }
        ...
        if len(candidates) != 200 or len(set(messages)) != 200:
            raise RuntimeError(f"repeats={repeats}: candidate contract failed")
        if any(len(message) != 1 for message in messages):
            raise RuntimeError(f"repeats={repeats}: expected one user message")
    ...
    payload = {
        ...
        "source": "arXiv:2506.24056v2 greedy covering applied to stop-vs-channel",
        "target_text": TARGET,
