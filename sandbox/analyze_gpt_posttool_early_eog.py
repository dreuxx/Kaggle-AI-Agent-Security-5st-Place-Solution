#!/usr/bin/env python3
"""Measure GPT-OSS EOG likelihood after each real post-tool output prefix.

This is a state diagnostic, not a replay.  It constructs the exact 200
AttackCandidate objects emitted by the current attack.py and renders candidate
zero through the SDK's real RuntimeHistory/tool schema path.
"""
...
OUTPUT = ROOT / "sandbox/output/gpt_posttool_early_eog_20260828.json"
NORMAL_POST = "<|channel|>final<|message|>"
EOG_TEXTS = ("<|return|>", "<|call|>")
...
def main() -> None:
    ...
    candidates = attack.AttackAlgorithm(
        config={"fixed_model": "gpt_oss", "n_candidates": 200}
    ...
    if len(candidates) != 200 or len({tuple(value) for value in serialized}) != 200:
        raise RuntimeError("Current attack.py does not emit 200 unique GPT candidates")
        ...
        positions.append(
            {
                ...
                "argmax": {
                    ...
                    "is_eog": argmax_id in set(eog_ids.values()),
                ...
                "top": _top(llm, logits, set(eog_ids.values())),
            }
        )
