...
on_exit() {
    ...
    trap - EXIT
    if [[ ${status} -eq 0 ]]; then
        message="email.list -> email.send + last_user, Strict 200, GPT/Gemma terminó completo"
    else
        message="email.list -> email.send + last_user, Strict 200 terminó con error ${status}"
    fi
    printf '\n================================================================\n'
...
printf 'TERMINAL ACTIVA: email.list -> email.send + last_user; Strict; 200; GPT/Gemma\n'
...
run_case gpt_oss "${GPT_MODEL}" -1 email_seen_last_user200_gpt_strict
run_case gemma "${GEMMA_MODEL}" 24 email_seen_last_user200_gemma_strict
