...
on_exit() {
    status=$?
    trap - EXIT
    printf '\n[%s] GPT unconditional A/B termino con estado %s\n' "$(date -Is)" "${status}"
    exit "${status}"
}
trap on_exit EXIT
...
run_case() {
    ...
    env SANDBOX_ALLOW_GPU=1 sandbox/run.sh \
        ...
        gpt_oss "${GPT_MODEL}" \
        --fixed-experiment-arm gpt_hybrid_k3_lss3 \
        --hard-cap 10 \
        --replay-limit 10 \
        --replay-budget 900 \
        --n-gpu-layers -1 \
...
printf '[4/5] GPT unconditional / Optimal\n'
run_case gpt_hybrid_k3_lss3_current_s10_optimal optimal

printf '[5/5] GPT unconditional / strict\n'
run_case gpt_hybrid_k3_lss3_current_s10_strict strict
