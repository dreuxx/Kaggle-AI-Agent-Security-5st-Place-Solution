...
EXPECTED_SHA="d5eaf8b30e0e43734ba39a5064d29625ecd7a6cfd83234b9c4c6c661e9bbcc7e"

on_exit() {
    status=$?
    trap - EXIT
    printf '\n================================================================\n'
    printf 'ALERTA TERMINAL: URL alfa minima 200 termino con estado %s\n' "${status}"
    printf '================================================================\n'
    if command -v notify-send >/dev/null 2>&1; then
        notify-send "JED replay" "URL alfa minima 200 termino con estado ${status}" || true
    fi
    exit "${status}"
}
trap on_exit EXIT
...
cd "${PROJECT_DIR}"
printf 'TERMINAL ACTIVA: URL alfa minima; Optimal; 200 GPT + 200 Gemma\n'
...
run_case gpt_oss "${GPT_MODEL}" -1 minimal_alpha_url200_gpt_optimal
run_case gemma "${GEMMA_MODEL}" 24 minimal_alpha_url200_gemma_optimal
