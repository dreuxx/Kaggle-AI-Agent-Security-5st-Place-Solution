...
on_exit() {
    ...
    trap - EXIT
    if [[ ${status} -eq 0 ]]; then
        message="Secret real Base64 / persistent 200 terminó completo"
    else
        message="Secret real Base64 / persistent 200 terminó con error ${status}"
    fi
    printf '\n================================================================\n'
...
cd "${PROJECT_DIR}"
printf 'TERMINAL ACTIVA: secret real -> Base64 -> POST; persistent; 200; GPT/Gemma\n'
...
run_case() {
    ...
    env SANDBOX_ALLOW_GPU=1 sandbox/run.sh \
        ...
        --replay-guardrail persistent_provenance_private \
        --output "sandbox/output/${label}.json"
    printf '[%s] END %s\n' "$(date -Is)" "${label}"
}

run_case gpt_oss "${GPT_MODEL}" -1 real_secret_base64_200_gpt_persistent
run_case gemma "${GEMMA_MODEL}" 24 real_secret_base64_200_gemma_persistent
