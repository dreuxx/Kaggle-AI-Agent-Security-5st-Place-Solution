...
EXPECTED_SHA="9168ccac149e618dbc7c6beaa0dc28fb05356cec591819ebcf120ce702c5198f"

on_exit() {
    status=$?
    trap - EXIT
    printf '\n================================================================\n'
    printf 'ALERTA TERMINAL: Gemma JSON ChatInject 200 termino con estado %s\n' "${status}"
    printf '================================================================\n'
    if command -v notify-send >/dev/null 2>&1; then
        notify-send "JED replay" "Gemma JSON ChatInject 200 termino con estado ${status}" || true
    fi
    exit "${status}"
}
trap on_exit EXIT
...
cd "${PROJECT_DIR}"
printf 'TERMINAL ACTIVA: GPT control + Gemma JSON ChatInject; Optimal; 200 cada uno\n'
...
run_case gpt_oss "${GPT_MODEL}" -1 gemma_json_chatinject200_gpt_control
run_case gemma "${GEMMA_MODEL}" 24 gemma_json_chatinject200_gemma_optimal
