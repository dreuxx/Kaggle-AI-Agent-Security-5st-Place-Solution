...
on_exit() {
    ...
    trap - EXIT
    if [[ ${status} -eq 0 ]]; then
        message="HTTP+correo, un mensaje, Optimal 200 terminó completo"
    else
        message="HTTP+correo, un mensaje, Optimal 200 terminó con error ${status}"
    fi
    printf '\n================================================================\n'
...
cd "${PROJECT_DIR}"
printf 'TERMINAL ACTIVA: HTTP POST seguido de correo; un mensaje; Optimal; 200; GPT/Gemma\n'
...
run_case gpt_oss "${GPT_MODEL}" -1 heterogeneous_single200_gpt_optimal
run_case gemma "${GEMMA_MODEL}" 24 heterogeneous_single200_gemma_optimal
