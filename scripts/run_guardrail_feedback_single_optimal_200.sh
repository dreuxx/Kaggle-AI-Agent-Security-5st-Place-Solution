...
on_exit() {
    ...
    trap - EXIT
    if [[ ${status} -eq 0 ]]; then
        message="Hallazgo 27 single / Optimal 200 terminó completo"
    else
        message="Hallazgo 27 single / Optimal 200 terminó con error ${status}"
    fi
    printf '\n================================================================\n'
...
cd "${PROJECT_DIR}"
printf 'TERMINAL ACTIVA: hallazgo 27 single, Optimal, 200 por modelo\n'
...
run_case gpt_oss "${GPT_MODEL}" -1 feedback_single200_gpt_optimal
run_case gemma "${GEMMA_MODEL}" 24 feedback_single200_gemma_optimal
