...
on_exit() {
    ...
    trap - EXIT
    printf '\n================================================================\n'
    printf 'ALERTA TERMINAL: correo->HTTP Optimal 200 termino con estado %s\n' "${status}"
    printf '================================================================\n'
    if command -v notify-send >/dev/null 2>&1; then
        notify-send "JED replay" "correo->HTTP Optimal 200 termino con estado ${status}" || true
    fi
    exit "${status}"
}
trap on_exit EXIT
...
cd "${PROJECT_DIR}"
printf 'TERMINAL ACTIVA: correo -> http.post; un mensaje; Optimal; 200; GPT/Gemma\n'
...
run_case gpt_oss "${GPT_MODEL}" -1 reverse_email_http200_gpt_optimal
run_case gemma "${GEMMA_MODEL}" 24 reverse_email_http200_gemma_optimal
