...
EXPECTED_SHA="496f746102e019ffd6095a4671e401fe521eecc796eb9fb8702009f92c2c90de"

on_exit() {
    status=$?
    trap - EXIT
    printf '\n================================================================\n'
    printf 'ALERTA TERMINAL: Gemma bare ChatInject 200 termino con estado %s\n' "${status}"
    printf '================================================================\n'
    if command -v notify-send >/dev/null 2>&1; then
        notify-send "JED replay" "Gemma bare ChatInject 200 termino con estado ${status}" || true
    fi
    exit "${status}"
}
trap on_exit EXIT
...
cd "${PROJECT_DIR}"
printf 'TERMINAL ACTIVA: GPT control + Gemma bare ChatInject; Optimal; 200 cada uno\n'
...
run_case gpt_oss "${GPT_MODEL}" -1 gemma_bare_chatinject200_gpt_control
run_case gemma "${GEMMA_MODEL}" 24 gemma_bare_chatinject200_gemma_optimal
