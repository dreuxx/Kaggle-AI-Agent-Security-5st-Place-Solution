...
EXPECTED_SHA="dbc68e66eb8a0ddf7ad3d36ef7af1dea3b610f714ca06b5f670a1367a7ece925"

on_exit() {
    status=$?
    trap - EXIT
    printf '\n================================================================\n'
    printf 'ALERTA TERMINAL: reasoning budget 100, Optimal 200 termino con estado %s\n' "${status}"
    printf '================================================================\n'
    exit "${status}"
}
trap on_exit EXIT
...
cd "${PROJECT_DIR}"
printf 'TERMINAL ACTIVA: maximo 100 reasoning tokens por prompt, Optimal, 200, GPT y Gemma\n'
...
run_case gpt_oss "${GPT_MODEL}" -1 reasoning_budget100_200_gpt_optimal
run_case gemma "${GEMMA_MODEL}" 24 reasoning_budget100_200_gemma_optimal
