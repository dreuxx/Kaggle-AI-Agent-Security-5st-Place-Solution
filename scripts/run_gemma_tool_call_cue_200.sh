...
EXPECTED_SHA="4d9dca302c547a5f73a72847f46015bf02b38a29798195debdd861434453ac7e"

on_exit() {
    status=$?
    trap - EXIT
    printf '\n================================================================\n'
    printf 'ALERTA TERMINAL: Gemma tool-call cue 200 termino con estado %s\n' "${status}"
    printf '================================================================\n'
    if command -v notify-send >/dev/null 2>&1; then
        notify-send "JED replay" "Gemma tool-call cue 200 termino con estado ${status}" || true
    fi
    exit "${status}"
}
trap on_exit EXIT
...
cd "${PROJECT_DIR}"
printf 'TERMINAL ACTIVA: GPT control + Gemma tool-call cue; Optimal; 200 cada uno\n'
...
run_case gpt_oss "${GPT_MODEL}" -1 gemma_tool_call_cue200_gpt_control
run_case gemma "${GEMMA_MODEL}" 24 gemma_tool_call_cue200_gemma_optimal
