...
EXPECTED_SHA="f12574de52edb91ef0eef51287975f75809db4a1e61aed592b72aef1a2e33e53"

on_exit() {
    status=$?
    trap - EXIT
    printf '\n================================================================\n'
    printf 'ALERTA TERMINAL: URL minima unica 200 termino con estado %s\n' "${status}"
    printf '================================================================\n'
    if command -v notify-send >/dev/null 2>&1; then
        notify-send "JED replay" "URL minima unica 200 termino con estado ${status}" || true
    fi
    exit "${status}"
}
trap on_exit EXIT
...
cd "${PROJECT_DIR}"
printf 'TERMINAL ACTIVA: URL minima unica; Optimal; 200 GPT + 200 Gemma\n'
...
run_case gpt_oss "${GPT_MODEL}" -1 minimal_unique_url200_gpt_optimal
run_case gemma "${GEMMA_MODEL}" 24 minimal_unique_url200_gemma_optimal
