...
on_exit() {
    ...
    trap - EXIT
    printf '\n================================================================\n'
    printf 'ALERTA TERMINAL: baseline original Optimal 200 termino con estado %s\n' "${status}"
    printf '================================================================\n'
    exit "${status}"
}
trap on_exit EXIT
...
cd "${PROJECT_DIR}"
printf 'TERMINAL ACTIVA: original rapido, Optimal, 200, GPT y Gemma\n'
...
run_case gpt_oss "${GPT_MODEL}" -1 original_now200_gpt_optimal
run_case gemma "${GEMMA_MODEL}" 24 original_now200_gemma_optimal
