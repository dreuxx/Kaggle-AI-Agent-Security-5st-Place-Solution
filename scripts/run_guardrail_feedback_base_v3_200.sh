...
on_exit() {
    ...
    trap - EXIT
    if [[ ${status} -eq 0 ]]; then
        message="Hallazgo 27 v3 base limpio, matriz 200, terminó completa"
    else
        message="Hallazgo 27 v3 base limpio, matriz 200, terminó con error ${status}"
    fi
    printf '\n================================================================\n'
...
cd "${PROJECT_DIR}"
printf 'TERMINAL ACTIVA: hallazgo 27 v3 base limpio; 200; GPT/Gemma; Optimal/Strict\n'
...
run_case gpt_oss "${GPT_MODEL}" -1 optimal feedback_base_v3_200_gpt_optimal
run_case gpt_oss "${GPT_MODEL}" -1 strict feedback_base_v3_200_gpt_strict
run_case gemma "${GEMMA_MODEL}" 24 optimal feedback_base_v3_200_gemma_optimal
run_case gemma "${GEMMA_MODEL}" 24 strict feedback_base_v3_200_gemma_strict
