...
OUTPUT_DIR="${PROJECT_DIR}/sandbox/output"

on_exit() {
    status=$?
    trap - EXIT
    if [[ ${status} -eq 0 ]]; then
        message="Private-frontier terminado correctamente"
    else
        message="Private-frontier terminó con error ${status}"
    fi
    printf '\n[%s] %s\n' "$(date -Is)" "${message}"
...
printf '[4/7] GPT router / Optimal\n'
run_case \
    gpt_guardrail_adaptive_s5_optimal \
    gpt_oss "${GPT_MODEL}" gpt_guardrail_adaptive optimal 5 -1

printf '[5/7] GPT router / strict\n'
run_case \
    gpt_guardrail_adaptive_s5_strict \
    gpt_oss "${GPT_MODEL}" gpt_guardrail_adaptive strict 5 -1

printf '[6/7] Gemma router / Optimal\n'
run_case \
    gemma_guardrail_adaptive_s3_optimal \
    gemma "${GEMMA_MODEL}" gemma_guardrail_adaptive optimal 3 24

printf '[7/7] Gemma router / strict\n'
run_case \
    gemma_guardrail_adaptive_s3_strict \
    gemma "${GEMMA_MODEL}" gemma_guardrail_adaptive strict 3 24
